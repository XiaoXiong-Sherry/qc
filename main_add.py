...
config_flags.DEFINE_config_file('agent', 'agents/acfql.py', lock_config=False) # 从其他Python 文件中加载额外的配置参数 # lock_config 决定了从配置文件加载的参数是否可以被进一步的命令行标志覆盖。
...
def main(_):
    ...
    config["horizon_length"] = FLAGS.horizon_length  # horizon_length 指的是动作分块（action chunking）的长度
    ...
    for i in tqdm.tqdm(range(1, FLAGS.offline_steps + 1)):
        ...
        if i == FLAGS.offline_steps - 1 or \
            ...
            eval_info['videos'] = get_wandb_video(renders)
